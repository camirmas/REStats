...
def power_curve_data():
    ...
    power = np.piecewise(
        ...
        [
            0,
            lambda x: rated_power
            * ((x - cut_in_speed) / (rated_speed - cut_in_speed)) ** 3,
            rated_power,
            0,
        ],
    )
...
def test_filter_outliers(power_curve_data):
    ...
    assert not (
        (filtered_df["Wind speed (m/s)"] > 15) & (filtered_df["Power (kW)"] < 1000)
    ).any()
...
def test_standardize(sample_dataframe):
    ...
    np.testing.assert_almost_equal(
        standardized_df["wind_dir"].values, expected_wind_dir.values, decimal=6
    )
...
def test_downsample():
    ...
    date_rng = pd.date_range(
        start="2020-01-01", end="2020-01-02", freq="10min", inclusive="left"
    )
    ...
    # Check if the downsampled wind_speed values are close to the mean of the
    # original data
    for time, group in df.groupby(df.index.hour):
        expected_wind_speed = group["wind_speed"].mean()
        assert downsampled_df.loc[
            group.index[0].floor("H"), "wind_speed"
        ] == pytest.approx(expected_wind_speed, abs=1e-6)

    # Check if the downsampled wind_dir values are close to the circular mean of the
    # original data
    for time, group in df.groupby(df.index.hour):
        expected_wind_dir = circular_mean(group["wind_dir"].values)
        assert downsampled_df.loc[
            group.index[0].floor("H"), "wind_dir"
        ] == pytest.approx(expected_wind_dir, abs=1e-6)

    # Check if the downsampled power values are close to the mean of the original data
    for time, group in df.groupby(df.index.hour):
        expected_power = group["power"].mean()
        assert downsampled_df.loc[group.index[0].floor("H"), "power"] == pytest.approx(
            expected_power, abs=1e-6
        )

    # Check if the downsampled turbulence_intensity values are close to the mean of the
    # original data
    for time, group in df.groupby(df.index.hour):
